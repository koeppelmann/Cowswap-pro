...
COLL = sys.argv[4] if len(sys.argv) > 4 else WETH    # buy / Aave collateral
DEBT = sys.argv[5] if len(sys.argv) > 5 else WXDAI   # sell / Aave debt
EMODE = int(sys.argv[6]) if len(sys.argv) > 6 else 0
...
if int(cast("call", DEBT, "allowance(address,address)(uint256)", owner, RELAYER, "--rpc-url", RPC).split()[0] or 0) < need:
    send(DEBT, "approve(address,uint256)", RELAYER, need, "--private-key", owner_key); time.sleep(2)
...
lev = {"sellToken": DEBT, "buyToken": COLL, "receiver": safe, "sellAmount": str(flash), "buyAmount": str(buyMin), "validTo": validTo,
