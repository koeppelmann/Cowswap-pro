...
MODULE = "0xbd913B8626DD7ACe1810E1797C93f27dD7906A5C"  # v4: stop trigger (HF<x) + receiver sweep
...
RETARGET_TYPE = ("Retarget(address safe,uint256 nonce,uint256 deadline,uint8 mode,address collateral,"
                 ...
                 "uint32 orderValidTo,uint256 minHealthFactor,address receiver,uint256 triggerHealthFactor)")
...
def build_retarget(mode, safe, slippage_bps=2000, fraction_bps=10000, extra=None, receiver="0x0000000000000000000000000000000000000000", trigger=0):
    ...
    if mode == 0:  # REDUCE (close / partial / decrease-lev)
        ...
        r = dict(safe=safe, nonce=int(time.time()), deadline=deadline, mode=0, collateral=WETH, debt=WXDAI,
                 ...
                 orderValidTo=validTo, minHealthFactor=0, receiver=receiver, triggerHealthFactor=trigger)
        ...
        r = dict(safe=safe, nonce=int(time.time()), deadline=deadline, mode=1, collateral=WETH, debt=WXDAI,
                 ...
                 orderValidTo=validTo, minHealthFactor=1050000000000000000, receiver="0x0000000000000000000000000000000000000000", triggerHealthFactor=0)  # HF >= 1.05 postcondition
    return r
...
def sign_intent(r, owner_key):
    struct_hash = keccak(abi_encode(
        ["bytes32","address","uint256","uint256","uint8","address","address","uint256","uint256","uint256","uint256","uint32","uint256","address","uint256"],
        [keccak(text=RETARGET_TYPE), r["safe"], r["nonce"], r["deadline"], r["mode"], r["collateral"], r["debt"],
         r["sellAmount"], r["repayAmount"], r["minBuy"], r["flash"], r["orderValidTo"], r["minHealthFactor"], r["receiver"], r["triggerHealthFactor"]]))
...
def tuple_of(r):
    return (r["safe"], r["nonce"], r["deadline"], r["mode"], r["collateral"], r["debt"], r["sellAmount"],
            r["repayAmount"], r["minBuy"], r["flash"], r["orderValidTo"], r["minHealthFactor"], r["receiver"], r["triggerHealthFactor"])
...
if __name__ == "__main__":
    ...
    # relay execute()
    T = "(address,uint256,uint256,uint8,address,address,uint256,uint256,uint256,uint256,uint32,uint256,address,uint256)"
