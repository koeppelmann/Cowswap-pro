 ...
 *
 * Run: GNOSIS_RPC=https://rpc.gnosischain.com forge test --match-path test/CowLeverageChain.fork.t.sol -vv
 */
...
contract CowLeverageChainForkTest is Test {
    ...
    {
        // order uids are derived from settleData by the flash wrapper itself
    ...
    function test_close_via_double_wrapper() public {
        ...
        assertGt(recovered, 90e18, "equity recovered (~100 minus premiums/slippage)");
        assertEq(IERC20(WXDAI).balanceOf(address(flashWrapper)), 0, "flash repaid exactly");
    ...
    function test_reject_thirdParty_flashloan_callback() public {
        ...
        vm.expectRevert(); // NotSelfInitiated (bubbled through Aave)
        IAavePoolFL(POOL).flashLoan(address(flashWrapper), assets, amounts, modes, address(0xBAD),
            abi.encode(bytes(""), bytes(""), recipients), 0);
    }
    ...
    function test_reject_direct_executeOperation() public {
        ...
        vm.expectRevert(); // NotPool
        flashWrapper.executeOperation(assets, amounts, premiums, address(flashWrapper), abi.encode(bytes(""), bytes(""), new address[](0)));
    }
    ...
    function test_reject_tampered_post_in_chain() public {
        ...
        vm.prank(solver);
        vm.expectRevert(); // post mismatch inside CoWSafeWrapper → whole chain reverts
        flashWrapper.wrappedSettle(_settleCd(WXDAI, WETH, FLASH, BUY_WETH), chain);
    }
...
contract FakeWrapper {
    ...
    function wrappedSettle(bytes calldata, bytes calldata) external returns (bytes4) {
        ...
        return this.wrappedSettle.selector;
    }
}
