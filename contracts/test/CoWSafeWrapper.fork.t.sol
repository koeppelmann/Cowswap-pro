 ...
 * Run: GNOSIS_RPC=https://rpc.gnosischain.com forge test --match-path test/CoWSafeWrapper.fork.t.sol -vv
 */
...
contract CoWSafeWrapperForkTest is Test {
    ...
    function _createSafe(uint256 salt) internal returns (address s) {
        ...
        owners[0] = owner;
        bytes memory initData = abi.encodeWithSelector(LevSafeInit.setup.selector, address(wrapper), WXDAI, vaultRelayer);
