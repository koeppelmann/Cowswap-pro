...
interface ISafeDomain { function domainSeparator() external view returns (bytes32); }

contract CoWSafeSigHandlerSim2 is DefaultCallbackHandler, HandlerContext, ISignatureValidator {
    ...
    // GPv2 constants (for self-validating "trigger" orders)
    ...
    /// Max sell for a self-authorized trigger order. A trigger is a SAME-TOKEN order with receiver ==
    /// the Safe, so value cannot leave; capping the size bounds any dust round-trip an attacker forces.
    uint256 public constant TRIGGER_MAX_SELL = 1e15;
    ...
    /// GPv2 order shape supplied (by anyone) in a trigger order's `signature` so the handler can
    /// re-derive its digest and validate it without an EOA signature. Self-describing, like ComposableCoW.
    struct TriggerOrder {
    ...
    /// True iff `_dataHash` is the digest of a self-authorized trigger order for `safe`:
    /// same-token, receiver == safe (value round-trips), sell <= cap, sell kind.
    function _isTrigger(address safe, bytes32 _dataHash, bytes calldata _signature) internal view returns (bool) {
        if (_signature.length == 0) return false;
        TriggerOrder memory o = abi.decode(_signature, (TriggerOrder));
        if (o.sellToken != o.buyToken) return false;
        if (o.receiver != safe) return false;
        if (o.sellAmount == 0 || o.sellAmount > TRIGGER_MAX_SELL) return false;
        if (o.kind != KIND_SELL) return false;
        bytes32 structHash = keccak256(abi.encode(
            ORDER_TYPE_HASH, o.sellToken, o.buyToken, o.receiver, o.sellAmount, o.buyAmount,
            o.validTo, o.appData, o.feeAmount, o.kind, o.partiallyFillable, o.sellTokenBalance, o.buyTokenBalance
        ));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", ISafeDomain(SETTLEMENT).domainSeparator(), structHash));
        return digest == _dataHash;
    }
    ...
    function isValidSignature(bytes32 _dataHash, bytes calldata _signature) external view returns (bytes4) {
        ...
        if (_msgSender() == SETTLEMENT) {
            ...
            // Bless path: leverage order, enforced pre/post via CoWSafeWrapper.
            if (ICoWSafeWrapper(WRAPPER).isBlessed(safe, _dataHash)) return UPDATED_MAGIC_VALUE;
            // Trigger path: harmless same-token dust order to self, self-describing in the signature,
            // so anyone can submit it (no EOA sig). Its pre-interactions deploy the Safe + register.
            if (_isTrigger(safe, _dataHash, _signature)) return UPDATED_MAGIC_VALUE;
            return FAIL;
        }
