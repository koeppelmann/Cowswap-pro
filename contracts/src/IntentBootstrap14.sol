...
contract IntentBootstrap14 {
    ...
    struct Intent {
        address owner;
        uint256 equity;   // delivered to the Safe by the carrier trade (in `debt` units)
        uint256 flash;    // flash-borrowed + sold (in `debt` units)
        uint256 buyMin;   // min collateral out
        uint256 borrow;   // debt borrowed from Aave
    ...
    {
        // equity is pre-funded in the Safe by the user's single transfer; pre only sets the sell approval
        bytes memory preCalls = abi.encodePacked(
            _ms(it.debt, abi.encodeWithSignature("approve(address,uint256)", RELAYER, it.flash))
        );
        ...
        public pure returns (bytes memory flData, bytes memory safeData)
    {
        ...
        loans[0] = Loan({ token: it.debt, amount: it.flash, recipient: safe });
    ...
    function uid(Intent memory it, address safe) public view returns (bytes memory) {
        ...
        bytes32 structHash = keccak256(abi.encode(
            ORDER_TYPE_HASH, it.debt, it.collateral, safe, it.flash, it.buyMin, it.validTo, appHash, uint256(0),
